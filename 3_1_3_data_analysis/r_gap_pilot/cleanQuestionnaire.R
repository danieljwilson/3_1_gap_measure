...
cleanQuestionnaire <- function(data, subjectCol = 1, scaleName, scaleMin = 1, scaleMax = 7, subscales = list(), itemsToReverse = c(), checkReliability = TRUE) {
  ...
  scaleName=''
  checkScaleItems <- data.table(tibble(scale = scaleName, itemName = colnames(dataWithoutPNo), item = as.numeric(1:ncol(dataWithoutPNo)), toReverse = 'no'))
  ...
  dataLong <- arrange(data.table((gather(data, item, score, -subjectCol))), pNo)
